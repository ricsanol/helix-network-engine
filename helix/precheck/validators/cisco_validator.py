...
from helix.precheck.commands.cisco_commands import CiscoCommands
from helix.precheck.validators.base import BasePreCheckValidator


class CiscoPreCheckValidator(BasePreCheckValidator):
    ...
    def _validate_physical_interface(self, interface_name: str) -> ValidationItem:
        output = self.client.send_command(CiscoCommands.interface_exists(interface_name))
        normalized_output = self._normalize_output(output)
        normalized_interface = interface_name.strip().lower()

        interface_found = (
            "invalid input" not in normalized_output
            and "does not exist" not in normalized_output
            and "not found" not in normalized_output
    ...
    def _validate_subinterface_not_exists(self, subinterface_name: str) -> ValidationItem:
        output = self.client.send_command(
            CiscoCommands.subinterface_exists(subinterface_name)
        )
        normalized_output = self._normalize_output(output)
        ...
        already_exists = (
            "invalid input" not in normalized_output
            and f"interface {normalized_subinterface}" in normalized_output
        )
    ...
    def _validate_vrf_exists(self, vrf_name: Optional[str]) -> ValidationItem:
        if not vrf_name or not vrf_name.strip():
            return ValidationItem(
                name="vrf_exists",
                passed=True,
                severity="WARNING",
                message="VRF não informada para validação. Check ignorado neste cenário.",
                details=None,
            )

        output = self.client.send_command(CiscoCommands.vrf_exists(vrf_name))
        normalized_output = self._normalize_output(output)
        ...
        return ValidationItem(
            ...
            severity="ERROR",
            message=(
                f"VRF {vrf_name} encontrada."
                if exists
                else f"VRF {vrf_name} não encontrada."
            ),
            details=output,
        )